	...
	const clickFollow = async () => {
		...
		if (currentToken) {
			const { data } = await client.put(
				`/api/follow?id=${artist.id}&type=artist`
			);
			if (data === 'success') {
				setNewFollow(true);
			}
	...
	const clickunFollow = async () => {
		...
		if (currentToken) {
			const { data } = await client.delete(
				`/api/unfollow?id=${artist.id}&type=artist`
			);
			if (data === 'success') {
				setNewFollow(false);
			}
	...
	return (
		<div className='artist-outer-div'>
			...
			<img
				...
				alt=''
				className='profile-pic artist-pic'
			/>
			<div className='artist-details'>
				...
				<div>
					<p>
						<Followers num={artist.followers.total} />
					</p>
					<p className='artist-label'>Followers</p>
				</div>
			</div>
			{newFollow ? (
				<div onClick={clickunFollow} className='playlist-created'>
					<Checkmark size={20} />{' '}
					<p style={{ marginLeft: '10px' }}>Following</p>
				</div>
			) : (
				<button onClick={clickFollow} className='btn playlist-btn'>
					Follow
				</button>
			)}
			<h3>Top Tracks</h3>
